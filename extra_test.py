...
# Helpfunction for getting predecessors of a specific Node in the .Dot file
def predecessors(graph, node_name):
...
def successors(graph, node_id):
    ...
    for edge in graph.get_edges():
        ...
        if edge.get_source() == target_node:
        ...
        if len(marked_c_nodes) > 0:
            ...
            if double_edged_reactions:
                for r_node in double_edged_reactions:  # For all Reaction nodes
                    ...
                    non_marked_reaction_predecessors = [e_node for e_node in predecessors(graph, r_node) if
                                                        graph.nodes[e_node]['type'] == "E"]
