...
file_path = 'C:\\Users\\marsh\\Documents\\GitHub\\BachelorQUBOProject\\large_dots_stats.xlsx'
...
plt.bar(x_positions, df['Nr. of Nodes'], color='skyblue', label='Nr. of Nodes', width=0.4)


# Adding some chart elements for clarity
plt.title('Comparison of Base Values and Total Values by Qutie and the new Method')
plt.xlabel('Metabolic Networks (large)')
plt.ylabel('Nr of Binary Variables')
plt.xticks([loc - 0.5 for loc in x_positions], df['File Name'], rotation=50)  # Set x-ticks to be the sorted row identifiers; rotate for readability
plt.legend()
